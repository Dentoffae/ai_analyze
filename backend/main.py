...
async def parse_demo(request: ParseDemoRequest):
    ...
    try:
        ...
        # Selenium Chrome: открытие страницы, скриншот и извлечение всего контента
        logger.info("  🔍 Запуск парсинга через Selenium...")
        ...
        logger.info(f"  📌 Title: {parsed.title[:50] if parsed.title else 'N/A'}...")
        logger.info(f"  📌 H1: {parsed.h1[:50] if parsed.h1 else 'N/A'}...")
        logger.info(f"  📌 Контент: {len(parsed.page_content) if parsed.page_content else 0} символов")
        logger.info(
            f"  📌 Screenshot: {len(parsed.screenshot_bytes) / 1024:.1f} KB"
            if parsed.screenshot_bytes else "  📌 Screenshot: N/A"
        )

        screenshot_base64 = (
            parser_service.screenshot_to_base64(parsed.screenshot_bytes)
            if parsed.screenshot_bytes else None
        )

        # Анализ: скриншот + весь контент страницы
        ...
        if screenshot_base64:
            ...
            analysis = await openai_service.analyze_parsed_content(
                ...
                title=parsed.title,
                h1=parsed.h1,
                paragraph=parsed.first_paragraph,
            )
        ...
        history_service.add_entry(
            ...
            response_summary=analysis.summary[:100] if analysis.summary else f"Title: {parsed.title or 'N/A'}"
        )
