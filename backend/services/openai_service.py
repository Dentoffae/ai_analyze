...
- Оценивай: цветовую палитру, типографику, композицию, UX/UI элементы"""
    ...
    async def analyze_parsed_content(
        ...
        """Анализ распарсенного контента сайта (fallback без скриншота)"""
        ...
        logger.info(f"  Контент: {len(page_content) if page_content else 0} символов")

        combined_text = page_content or ""
    ...
    async def analyze_website_screenshot(
        ...
        """Комплексный анализ сайта: скриншот + весь текстовый контент страницы"""
        logger.info("=" * 50)
        ...
        if first_paragraph:
            context_parts.append(f"Первый абзац: {first_paragraph}")
        if page_content:
            context_parts.append(f"Полный контент страницы:\n{page_content}")

        context = "\n\n".join(context_parts)
        logger.debug(f"  Контекст: {len(context)} символов")

        system_prompt = """Ты — эксперт по конкурентному анализу и UX/UI дизайну. Проанализируй скриншот сайта конкурента вместе с полным текстовым контентом страницы и верни структурированный JSON-ответ.
                        ...
                        "content": [
                            {
                                ...
                                "text": (
                                    ...
                                    "Используй и скриншот, и весь текстовый контент страницы:\n\n"
                                    f"{context}"
                                )
                            },
                            {
                                "type": "image_url",
                                "image_url": {
                                    "url": f"data:{mime_type};base64,{screenshot_base64}"
                                }
                            }
                        ]
                    }
